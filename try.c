...
// Function to check if a word is a positive amplifier and update amplifying effect
bool checkAmplifier(char *word, float *All_cap, float *amplifying_effect) {
    ...
    for (int p = 0; p < num_P_amp; p++) {
        if (strcmp(word, P_amp[p]) == 0) {
            *amplifying_effect = boost_Fac * (*All_cap); // Apply the boost factor and capitalization effect
            return true; // Indicate that the word is an amplifier
        }
    }
    for (int n = 0; n < num_N_amp; n++) {
        if (strcmp(word, N_amp[n]) == 0) {
    ...
    return false; // Indicate that the word is not an amplifier
}
bool checkNegatin(char *word, float *All_cap, float *negating_effect) {
    ...
    for (int e = 0; e < num_negations; e++) {
        if (strcmp(word, negations[e]) == 0) {
            *negating_effect = negation_Fac * (*All_cap);
            return true; // Return true if it's a negation
        }
    }
    return false; // Return false if not a negation
}
...
float sentimentVader(WordData* data, int data_size, WordData* tokens, int token_count) {
    ...
    for (int i = 0; i < token_count; i++) {
        // Reset capitalization multiplier for each token
        All_cap = isAllCaps(tokens[i].word) ? 1.5 : 1.0;

        // Convert token to lowercase for uniform matching
        for (char *p = tokens[i].word; *p != '\0'; ++p) {
            *p = tolower(*p);
        }

        // Handle negation stoppers
        for (int s = 0; s < num_stoppers; s++) {
            if (strcmp(tokens[i].word, negation_stoppers[s]) == 0) {
                negating_effect = 1.0; // Reset negation effect
                break;
            }
        }

        // Check for amplifiers and negations
        bool is_amp = checkAmplifier(tokens[i].word, &All_cap, &amplifying_effect);
        bool is_neg = checkNegatin(tokens[i].word, &All_cap, &negating_effect);

        // Skip processing for effect words, but ensure their effects persist
        if (is_amp || is_neg) {
